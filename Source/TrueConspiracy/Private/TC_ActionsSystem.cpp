...
void TC_ActionsSystem::PlayCard(TC_GameStates& InGameState, const FAIActions& InAction)
{
	...
	// V�rification de la carte � jouer
	...
	// V�rification de la main du joueur
	...
	// V�rification du GamePlate et du plateau correspondant
	...
	//ActivePlayer->GetPlayerBoard()->GetBoardSlots()[InAction.BoardSlotIndex]->GetBoardSlotSlots()[InAction.BoardSlotCardIndex]->GetSlotCard()->SetSlot(InAction.PlayingSlot); //Attribution du slot � la carte
	...
	if (SpawnedCard->GetPlayer()->GetPlayerPhaseState() == ETC_PhaseState::Defense)
	{
		SpawnedCard->SetActorRelativeRotation(SpawnedCard->GetActorRotation() + FRotator(0.f, 180.f, 180.f));
	}
		SpawnedCard->Init();
		if (!SpawnedCard)
		{
			//UE_LOG(LogTemp, Warning, TEXT("PlayCard: �chec du spawn de la carte."));
			return;
		}
...
void TC_ActionsSystem::MoveCard(TC_GameStates& InGameState, const FAIActions& InAction)
{
	...
	//R�cup�ration du plateau de jeu
	ATC_Plate* Plate = InGameState.GetGamePlate();
	if (!Plate)
	{
	...
	//R�cup�ration du board du joueur actif
	ATC_Board* PlayerBoard = ActivePlayer->GetPlayerBoard();
	if (!PlayerBoard)
	{
	...
	//R�cup�ration des BoardSlots du joueur actif
	...
	//R�cup�ration du BoardSlot origine de la carte � d�placer
	...
	if (!CardToMove)
	{
		//UE_LOG(LogTemp, Warning, TEXT("MoveCard: Aucune carte trouv�e � la position sp�cifi�e."));
		return;
	}
...
void TC_ActionsSystem::ApplyAction(TC_GameStates& InGameState, const FAIActions& InAction)
{
	...
	case EActionType::DrawCard:
	{
		...
		// -> retirer la premi�re carte de la pioche
		// -> ajouter � la main
